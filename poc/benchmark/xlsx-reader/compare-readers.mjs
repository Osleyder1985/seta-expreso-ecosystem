...
async function exceljsRead(buffer){
  ...
  return wb.worksheets.map(s=>({name:s.name,state:s.state,rowCount:s.rowCount,columnCount:s.columnCount,merged:[...s.mergedCells],rows:s.actualRows.map(r=>r.values.slice(1).map(v=>typeof v==='object'&&v?.formula?{formula:v.formula,result:v.result,numFmt:v.numFmt}:v))}));
}
...
for(const file of fixtureFiles){
  ...
  for(const [reader,fn] of Object.entries(readers)){
    ...
    const warm=samples.slice(1).map(x=>x.durationMs).sort((a,b)=>a-b);
    results.push({fixture:file,reader,bytes:buffer.length,durationMedianMs:warm[Math.floor(warm.length/2)],durationSamplesMs:samples.map(x=>x.durationMs),maxRssDelta:Math.max(...samples.map(x=>x.rssDelta)),snapshotHash:error?null:hash(result),error});
  }
}
const report={node:process.version,platform:process.platform,arch:process.arch,fixtureCount:fixtureFiles.length,fixtures:fixtureFiles,results};
